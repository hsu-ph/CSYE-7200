...
object Function {
  ...
  // TO BE IMPLEMENTED 
    (t1: Try[T1], t2: Try[T2]) => for {v1 <- t1; v2 <- t2} yield f(v1, v2)
  ...
  // TO BE IMPLEMENTED 
    (t1: Try[T1], t2: Try[T2], t3: Try[T3]) => for {v1 <- t1; v2 <- t2; v3 <- t3} yield f(v1, v2, v3)
  ...
  // TO BE IMPLEMENTED 
    (t1: Try[T1], t2: Try[T2], t3: Try[T3], t4: Try[T4], t5: Try[T5], t6: Try[T6], t7: Try[T7]) =>
      for {v1 <- t1; v2 <- t2; v3 <- t3; v4 <- t4; v5 <- t5; v6 <- t6; v7 <- t7} yield f(v1, v2, v3, v4, v5, v6, v7)
