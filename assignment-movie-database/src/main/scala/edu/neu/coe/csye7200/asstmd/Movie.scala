...
import edu.neu.coe.csye7200.asstmd.Rating
...
object Movie extends App {
  ...
  def elements(list: Seq[String], indices: Int*): List[String] = {
    ...
    val result: Seq[String] = {
      ...
      // STUB
      val result: Seq[String] = indices.map(index => list(index))
      result.toList
      // END
    }
    result.toList
...
object Rating {
  ...
  def apply(s: String): Rating = {
    ...
    val rRating = """^(\w*)(-(\d\d))?$""".r
    s match {
      case rRating(code, null, null) => Rating(code, None) // No age provided
      case rRating(code, _, age) => Rating(code, Some(age.toInt)) // Age provided
      case _ => throw ParseException(s"Error in rating: $s") // Invalid input format
    }
    // END
  }
}
